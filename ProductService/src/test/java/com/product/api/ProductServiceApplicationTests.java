...
class ProductServiceApplicationTests {
	...
	public void getProductTest() {
		when(productRepository.findAll()).thenReturn(Stream
				.of(new Product(1, "Electronics","TV", "household",4, "good", 30000, null, null), new Product(2, "Electronics", "AC","household", 3, "better", 10000, null, null)).collect(Collectors.toList()));
		assertEquals(2, productController.getProduct().size());
	}
	//Check test for null list or empty list
	@Test 
	void getProductNull() {
	...
	public void addProductTest() {
		Product product = new Product(3, "Electronics","Heater", "household",5, "good", 15000, null, null);
		when(productRepository.save(product)).thenReturn(product);
		assertEquals(product, productController.addProduct(product));
	}
	// Check test for get all Products
	 @Test 
		void getUserTest1() {
			List<Product> List = new ArrayList<>();

			List.add(new Product(3, "Electronics","Heater", "household",5, "good", 15000, null, null));
			List.add(new Product(3, "Electronics","Heater", "household",5, "good", 15000, null, null));
			List.add(new Product(3, "Electronics","Heater", "household",5, "good", 15000, null, null));
	...
	//Check Test for delete product
		...
		public void deleteProductTest() {
			Product product = new Product(3, "Electronics","Heater", "household", 4, "good", 25000, null, null);
			when (productRepository.getById((long) 1)).thenReturn(product);
			productController.deleteProduct((long) 1);
	...
	public void deleteProductTest1() {
		...
		verify(productRepository, times(1)).deleteById(product);
	}
	
	//Check test for updating product
	public void UpdateProductTest() {
		Product product=new Product(3, "Electronics","Heater", "household", 4, "good", 25000, null, null);
		when(productRepository.getById((long) 1)).thenReturn(product);
		
		 String productType= "Electronics";
	     String productName="Heater";
	     String category="Household";
		 int rating=2;
	     String review="Better";
	  	 double price=12000;
		 String description=null;
         String specification=null;
         
         Product pro=new Product( 0, productType, productName, category, rating, review, price, description, specification);
	...
	void GetuserTest2() throws InvalidException {
		...
		for (Product p : list) {
			if (p.getId() == 0) {
				...
				Mockito.when(p.getProductType().equals(null)).thenThrow(InvalidException.class);

				assertNull(p.getProductType());

			} else if (p.getProductName().equals(null)) {

				Mockito.when(p.getProductName().equals(null)).thenThrow(InvalidException.class);
				...
				Mockito.when(p.getDescription().equals(null)).thenThrow(InvalidException.class);

				assertNull(p.getDescription());
				
			} else if (p.getSpecification().equals(null)) {

				Mockito.when(p.getSpecification().equals(null)).thenThrow(InvalidException.class);
